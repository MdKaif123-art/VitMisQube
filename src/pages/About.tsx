...
const About = () => (
  <div className="max-w-6xl mx-auto px-2 md:px-6">
    ...
    <div className="bg-black rounded-xl shadow p-8 mb-10 border border-[#00FFFF]">
      <h2 className="text-2xl font-bold text-center text-[#00FFFF] mb-2">Our Team</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {team.map((member) => (
          <div key={member.name} className="bg-black rounded-xl shadow border border-[#00FFFF] p-6 flex flex-col items-center text-center hover:shadow-[0_0_15px_rgba(0,255,255,0.3)] transition-all">
            <img src={member.img} alt={member.name} className="w-20 h-20 rounded-full object-cover mb-3 border-4 border-[#008080] shadow" />
            <div className="font-bold text-[#00FFFF] mb-1">{member.name}</div>
            <div className="text-white text-sm mb-1">{member.role}</div>
            <div className="text-white text-xs">{member.bio}</div>
          </div>
        ))}      
      </div>
    </div>
